...
LagrangeanFormulation::LagrangeanFormulation(vector< vector<double> >& p_A, vector<double>& p_b, 
                                             vector<double>& p_c,  vector<int>& p_op,
                                             int p_objective_type):Formulation(p_A,p_b,p_c,p_op,p_objective_type){
    init();
...
LagrangeanFormulation::LagrangeanFormulation(vector< vector<double> >& p_A, vector<double>& p_b, 
                                             ...
                                             vector<int>& p_no_lagrangean_restrictions):Formulation(p_A,p_b,p_c,p_op,p_objective_type){
    
    init();
    _no_lagrangean_restrictions = p_no_lagrangean_restrictions;
}

LagrangeanFormulation::LagrangeanFormulation(vector< vector<int> >& p_A_index, vector< vector<double> >& p_A_cost, 
                                             vector<double>& p_b, vector<double>& p_c,  vector<int>& p_op,
                                             int p_objective_type):
                                             Formulation(p_A_index,p_A_cost,p_b,p_c,p_op,p_objective_type){
    init();
    _no_lagrangean_restrictions = vector<int>();   
}
